<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"180rpx","fontSize":"28rpx","color":"#333","textAlign":"left"}'>关联表id：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.refid}}</view>
				</view>
                				                							                				<view class="detail-list-item" :style='{"boxShadow":"","margin":"","borderColor":"#fff","backgroundColor":"rgba(255, 255, 255, 0.56)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"68rpx"}'>
					<view class="lable" :style='{"width":"180rpx","fontSize":"28rpx","color":"#333","textAlign":"left"}'>用户名：</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusszhishidian');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discusszhishidian');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discusszhishidian', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscusszhishidian', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscusszhishidian/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discusszhishidian', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discusszhishidian', this.detail);
				this.$utils.msg('审核成功');
