<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="科目类型" prop="kemuleixing">
          <el-select v-model="ruleForm.kemuleixing" placeholder="请选择科目类型">
            <el-option
                v-for="(item,index) in kemuleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="重点度" prop="zhongdiandu">
          <el-select v-model="ruleForm.zhongdiandu" placeholder="请选择重点度">
            <el-option
                v-for="(item,index) in zhongdianduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="重点度" prop="zhongdiandu">
	      <el-input v-model="ruleForm.zhongdiandu"
                placeholder="重点度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kemuleixing = true;
          ...
          if(o=='zhongdiandu'){
            this.ruleForm.zhongdiandu = obj[o];
	    this.ro.zhongdiandu = true;
            continue;
          }
          if(o=='neirong'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.zhongdianduOptions = "一星,两星,三星".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `zhishidian/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springbootj11ei/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhishidian/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhishidianCrossAddOrUpdateFlag = false;
                  this.parent.search();
